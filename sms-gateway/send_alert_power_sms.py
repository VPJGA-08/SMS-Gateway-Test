...
def send_sms_to_number(ser, number, alert_power_message, logger):
    ...
    ser.write(alert_power_message.encode("ascii", errors="replace"))
...
def main():
    ...
    alert_power_message = cfg.get("SMS", "alert_power_message", fallback="").strip()
    ...
    if not recipients:
        ...
        sys.exit(2)
    if not alert_power_message:
        logger.error("Config error: set [SMS].alert_power_message in sms_config.ini")
        sys.exit(2)
    ...
    try:
        ...
        for i, number in enumerate(recipients, 1):
            ok, _ = send_sms_to_number(ser, number, alert_power_message, logger)
            successes += int(ok)
            if i < len(recipients):
