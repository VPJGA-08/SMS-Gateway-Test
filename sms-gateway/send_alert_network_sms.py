...
def send_sms_to_number(ser, number, alert_network_message, logger):
    ...
    ser.write(alert_network_message.encode("ascii", errors="replace"))
...
def main():
    ...
    alert_network_message = cfg.get("SMS", "alert_network_message", fallback="").strip()
    ...
    if not recipients:
        ...
        sys.exit(2)
    if not alert_network_message:
        logger.error("Config error: set [SMS].alert_network_message in sms_config.ini")
        sys.exit(2)
    ...
    try:
        ...
        for i, number in enumerate(recipients, 1):
            ok, _ = send_sms_to_number(ser, number, alert_network_message, logger)
            successes += int(ok)
            if i < len(recipients):
