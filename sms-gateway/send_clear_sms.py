...
def send_sms_to_number(ser, number, clear_message, logger):
    ...
    ser.write(clear_message.encode("ascii", errors="replace"))
...
def main():
    ...
    clear_message = cfg.get("SMS", "clear_message", fallback="").strip()
    ...
    if not clear_message:
        logger.error("Config error: set [SMS].clear_message in sms_config.ini")
        sys.exit(2)
    ...
    try:
        ...
        for i, number in enumerate(recipients, 1):
            ok, _ = send_sms_to_number(ser, number, clear_message, logger)
            successes += int(ok)
            if i < len(recipients):
